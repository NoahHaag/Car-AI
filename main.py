...
class Car(pygame.sprite.Sprite):
    ...
    def collision(self):
        ...
        collision_point_right_front = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle + 18)) * length),
             int(self.rect.center[1] - math.sin(math.radians(self.angle + 18)) * length)])
        collision_point_left_front = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle - 18)) * length),
             int(self.rect.center[1] - math.sin(math.radians(self.angle - 18)) * length)])

        collision_point_right_side = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle + 90)) * 20),
             int(self.rect.center[1] - math.sin(math.radians(self.angle + 90)) * 20)])

        collision_point_left_side = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle - 90)) * 20),
             int(self.rect.center[1] - math.sin(math.radians(self.angle - 90)) * 20)])

        collision_point_right_rear = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle - 160)) * length),
             int(self.rect.center[1] - math.sin(math.radians(self.angle - 160)) * length)])

        collision_point_left_rear = self.adjust_pos_against_bounds(
            [int(self.rect.center[0] + math.cos(math.radians(self.angle + 160)) * length),
             int(self.rect.center[1] - math.sin(math.radians(self.angle + 160)) * length)])
